...
name: signal-commit
description: >
  Instantly stages all changes and commits with an auto-generated conventional
  commit message derived from the diff. Zero prompts, zero confirmation by
  default. Use when user types /signal-commit, "commit everything", "just commit",
  "signal commit", or asks to commit current changes without specifying a message.
  Supports --draft (show message without committing) and --split (atomic commits
  per logical change).
signal_bundle_version: "0.1.0"
---
